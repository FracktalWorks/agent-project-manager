...
### Check one specific project list
```bash
# Penrose Pellet Extruder
python .github/skills/project-tracking/scripts/fetch_status.py --list-id 901611050642

# Julia Series
python .github/skills/project-tracking/scripts/fetch_status.py --list-id 901611246751

# MDS (Material Drying System)
python .github/skills/project-tracking/scripts/fetch_status.py --list-id 901612525485

# Quality Control
python .github/skills/project-tracking/scripts/fetch_status.py --list-id 901613553036
```

### Filter by person — see only one person's tasks
```bash
python .github/skills/project-tracking/scripts/fetch_status.py --all-projects --assignee "Suryansh"
python .github/skills/project-tracking/scripts/fetch_status.py --all-projects --assignee "Kiran"
python .github/skills/project-tracking/scripts/fetch_status.py --list-id 901612525485 --assignee "Ayush"
```

### Save status report to file
```bash
python .github/skills/project-tracking/scripts/fetch_status.py --all-projects \
    --output outputs/status_2026-06-06.json
```

### Output JSON schema
The script always outputs valid JSON. In `--all-projects` mode:
```json
{
  "fetched_at": "2026-06-06T...",
  "projects": [
    {
      "project_name": "MDS (Material Drying System)",
      "list_id": "901612525485",
      "tasks": [
        { "id": "...", "title": "...", "assignee": "Suryansh Lal",
          "due_date": "2026-06-10", "status": "to do", "flag": "at_risk" }
      ]
    }
  ]
}
...
| Flag | Meaning |
|------|---------|
| `on_track` | Due date is >3 days away |
| `at_risk` | Due within 3 days |
| `blocked` | Status contains "block" |
...
| Script | Purpose |
|---|---|
| `.github/skills/project-tracking/scripts/fetch_status.py` | Pull task status from ClickUp for a project/list |
| `.github/skills/project-tracking/scripts/generate_report.py` | Format a human-readable status report |
