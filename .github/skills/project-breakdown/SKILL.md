...
Once breakdown is complete, pass to `clickup-ops` skill with:
- One task per Work Package (Level 3 in WBS)
- Each task has: verb+object title, named owner (`clickup_user_id` from `data/hr_structure.json`), PERT Expected date, priority flag (Urgent if on critical path)
- Tasks on the critical path get `priority = 1` (Urgent)
- Mark external-dependency tasks with the dependency described in the task description
